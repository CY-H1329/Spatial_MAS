#!/usr/bin/env python3
"""
Inférence MindCube (MCQ) avec adaptateurs LoRA Qwen3-VL entraînés sur 3DSRBench.

Écrit timing_infer.jsonl (un JSON par ligne) + timing_infer_summary.json
"""
...
def _import_qwen():
    try:
        ...
        raise SystemExit(
            "Import Qwen3VL impossible. Mettez à jour transformers (voir README).\n" f"Détail: {e}"
        ) from e
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--adapter_dir", type=str, required=True, help="Dossier sorti par train_lora_qwen3vl.py")
    ...
    p.add_argument("--output_dir", type=str, default="./outputs/mindcube_infer")
...
def main() -> None:
    ...
    with open(jsonl_path, "w", encoding="utf-8") as fj:
        fj.write(
            json.dumps({"event": "model_load", "load_model_s": load_model_s}, ensure_ascii=False)
            + "\n"
        )
        for i, row in enumerate(tqdm(rows, desc="MindCube")):
